...
from LoadData import getData_cifar
from MainModel import MLP, CNN, flatten_list, unflatten_vector, caL_loss_acc, mean, gm
...
def centralized_rsa(setting, attack=None, save_model=False, save_results=False):
    """
    ...
    for i in range(sgdConfig['nodeSize']):
        model[i].load_state_dict(model0.state_dict())
        for para in model[i].parameters():
            para.data.add_(torch.rand_like(para).data - 0.5, alpha=0.01)

    train_dataset_subset, train_loader, test_loader = getData_cifar(setting, sgdConfig)
    train_loaders_splited = [
        torch.utils.data.DataLoader(dataset=subset, batch_size=sgdConfig['batchSize'], shuffle=True, pin_memory=False)
